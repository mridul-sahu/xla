...
void BuildSingleMetadataMap(const MetadataMap& from_metadata,
  ...
  // NOLINTNEXTLINE
  for (const auto& [from_id, from_metadata] : from_metadata) {
    if (from_metadata.name().empty()) {
      (*id_map)[from_id] = from_id;
      continue;
    }
    auto [it_meta, inserted] =
        to_by_name.try_emplace(from_metadata.name(), next_id);
    if (inserted) {
      (*to_metadata)[next_id] = from_metadata;
      next_id++;
    }
...
void RemapEventMetadata(tensorflow::profiler::XEvent* event,
