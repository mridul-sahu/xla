...
/**
 * @brief Finds the block-level parameters using the experimental TilingSpace
 * propagation framework.
 *
 * M and N block sizes are defined at the fusion root output. However, because
 * the output shape may be transposed, elementwise fused, or broadcasted, M and
 * N can correspond to arbitrary dimensions at the root. This function loops
 * through all pairs of output dimensions (i, j) to place `block_m` and
 * `block_n` respectively, and maps these candidate outputs along with `block_k`
 * to the fusion's tiling space. It verifies that all constraints are satisfied
 * and that backward-propagating the root tile results in the expected tile
 * sizes [1, ..., 1, block_m, block_n] at the core `dot` instruction.
 *
 * @param dot The core dot or scaled dot instruction.
 * @param config The Triton GEMM configuration containing block_m, block_n,
 * block_k.
 * @param mlir_context The MLIR context.
 * @return absl::StatusOr<BlockLevelParameters> The block-level parameters if
 * successful, or an error status if no valid output tile configuration
...
absl::StatusOr<BlockLevelParameters> FindBlockLevelParametersWithTilingSpace(
  ...
  for (int m = 0; m < root_rank; ++m) {
    for (int n = 0; n < root_rank; ++n) {
      if (m == n || (config.block_m == config.block_n && m > n)) {
        continue;
      }
      llvm::SmallVector<int64_t> parallel_tile_sizes(root_rank, 1);
      parallel_tile_sizes[m] = config.block_m;
      parallel_tile_sizes[n] = config.block_n;
      ...
      for (const DimensionInfo& dim : ts->dimensions()) {
        ...
        if (dim.type == DimensionSemantics::kSequential && dim.hlo == dot) {
